...
def salary():
    ...
    if request.method == "POST" and request.form['action'] == 'search':
        ...
        try:
            ...
            return render_template("salary.html", basic=basic, falw=falw, otherded=otherded, ot=ot, disc=disc, nsf=nsf, oalw=oalw, tax=tax, med=med, tran=tran, ntax=ntax, edf=edf, arr=arr, att=att, eoy=eoy, loan=loan, car=car, leave=leave, slevy=slevy, spebns=spebns, late=late, edurel=edurel, speprobns=speprobns, nps=nps, medrel=medrel, payable=payable, ded=ded, net=net, cgross=cgross, pgross=pgross, iet=iet, netch=netch, cpaye=cpaye, ppaye=ppaye, paye=paye, ecsg=ecsg, ensf=ensf, elevy=elevy, absence=absence, eid=eid, fname=fname, lname=lname, pos=pos, month=month, year=year, pnet=pnet)
    ...
    # Save To Database
        ...
        try:
            ...
            query1 = """UPDATE salary
                        ...
                        WHERE 
                        UNQ = %s;"""
            data1 = [fixedAlw, otherDed, overtime, discBns, NSF, otherAlw2, tax, medical, transport, overseas, ntax, edf, arrears, attendance, eoy, loan, car, leaveRef, slevy, speBns, lateness, educationRel, SpeProBns, NPS, medicalRel, Payable, Deduction, Net, NetPaysheet, cgross, pgross, iet, netch, cpaye, ppaye, paye, ecsg, ensf, elevy, absence, UNQ ]
        ...
        try:
            ...
            if eid == "PP01":
                ...
                for i in arrays:
                    ...
                    """
                    data1 = [eid, flname, basic , fixAllow, otherDed, ot, discBns, nsf, otherAllow, tax, medical, trans, overseas, ntax, edf, arrears, attBns, eoy, loan, car, leave, slevy, speBns, lateness, education, SpeProBns, nps, Medicalrel, payable, deduction, net, NetPaysheet, cgross, prevGross, iet, netch, cpaye, ppaye, paye, enps ,ensf, levy, ab, month, year, UNQ]
                    cursor.execute(insert_query, data1)
...
def paysheet():
    ...
    if request.method == "POST" and request.form['action'] == 'pdf':
        try:
            ...
            query1 = "SELECT EmployeeName, BasicSalary, Arrears, Overtime, LeaveRef, EOY, Transport, Overseas, OtherAllow, Payable, Absences, PAYE, NPS, NSFEmpee, Medical, SLevy, Lateness, Deduction, NetPaysheet FROM salary"
            cursor.execute(query1)
            data = cursor.fetchall()
            print(data)
            session["data"] = data
